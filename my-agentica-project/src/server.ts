...
// 대분류 목록 조회 엔드포인트
app.get("/api/ai/categories", (req: Request, res: Response) => {
  const categories = [
    "지역개발",
    "교육",
    "일반공공행정",
...
// 서비스 버전 정보 엔드포인트 (추가)
app.get("/api/ai/version", (req: Request, res: Response) => {
  res.json({
    success: true,
    data: {
      service: "Agentica AI Service",
...
app.use((req: Request, res: Response) => {
  ...
  res.status(404).json({
    ...
    suggestion:
      "Please check the available endpoints above and ensure you're using the correct HTTP method.",
  });
});
...
const server = app.listen(port, () => {
  ...
  console.log(
    `💡 Recommendations: http://localhost:${port}/api/ai/recommendations`
  );
