...
export class PublicDataService {
   ...
   */
  private extractFileDataName(prompt: string): string {
    const majorCategory = this.extractMajorCategory(prompt);
  ...
  private extractDateInfo(prompt: string): {
    year?: number | null; // ← null도 허용하도록 수정
    isRecent: boolean;
    dateRange?: {
      startYear: number;
      endYear: number;
   ...
   * 설명 텍스트 생성
   */
  private generateDescription(prompt: string, majorCategory: string): string {
    const year = this.extractYear(prompt);
    const agency = this.extractAgency(prompt);
   ...
   * 데이터 유효성 검증
   */
  private validateInput(input: { prompt: string }): boolean {
    const { prompt } = input;
   ...
   * 로깅용 메타데이터 생성
   */
  private generateMetadata(prompt: string): {
    ...
    if (hasDateFilter) confidence += 0.1;
    if (majorCategory !== "일반공공행정") confidence += 0.2;
